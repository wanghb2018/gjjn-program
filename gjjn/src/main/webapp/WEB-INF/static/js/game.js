...
function showalert(){
	// mui.alert("1.各位用手机的指挥官大佬，知道你们不看公告，如果资源不同步的话，请点击下方的按钮，例如碎片数量有问题的话，请点击一下仓库就会自动刷新的。\n2.新增魔方出售功能\n3.新增魔方购买功能","公告");
}
function login(){
...
function autoLogin(){
	mui.get("autoLogin",function(data){
		console.log(data);
		if(data.hr == 0){
			$('#zhop').hide();
			role = data.data
...
function renderRoleData(role){
	...
	$('#rolelevel').html("Lv."+role.level);
	$('#roletouxiang').attr('src',role.touxiang);
	$('#zs_num').html(role.zuanshi);
	$('#wz_num').html(role.wuzi);
	$('#roleid').html(role.id);
	$('#rolejunxian').html(role.junxian);
	$('#roleshiyou').html(role.shiyou);
	$('#rolemofang').html(role.mofang);
	$('#rolekeyandian').html(role.keyandian);
	$('#roleexp').html(role.exp);		
	$('#roleguajitime').html(role.guajitime);
	renderRolejy();
}

function renderRolejy(){
	if (rolesjs == null){
		$.ajax({
			url: 'getRolesjs',
			type: "GET",
	...
	var rolesj = rolesjs[role.level-1];
	console.log(rolesj);
	$('#roleexprate').html((role.exp*100/rolesj.needjy).toFixed(7)+'%');
	$('#rolesj').html(rolesj.needjy);	
}
function renderDuiwu(){
	mui.get('getDuiwu',function(data){
		console.log(data);
		if (data){
			var l = data.data.myjns;
			var liststr = "";
			for(var i=0;i<data.data.duiwu.count;i++){
				liststr = liststr + "<li class='mui-table-view-cell mui-media'><div style='float: left;'><img class='mui-media-object mui-pull-left' src='"+ l[i].touxiang+"'><div class='mui-media-body'><font color='"+l[i].color+"'>"+l[i].name+"</font><p class='mui-ellipsis'><font color='white'>Lv."+l[i].level+"</font><font style='margin-left: 5px;' color='white'>战斗力："+l[i].zdl+"</font></p></div></div><div style='float: right;'><button type='button' class='imglibutton mui-btn' onclick='jnxiuxi("+l[i].id+")'>休息</button> <button type='button' class='imglibutton mui-btn' onclick='jninfoview("+l[i].id+")'>查看</button></div></li>";
			}
			$('#rolezdl').html(data.data.duiwu.totalzdl);
			$('#mydwlist').empty();
...
function renderMaps(){
	...
	console.log(maps);
	var mapstr = "";
	for(var i=0;i<role.openmapId;i++){
	...
	$('#mapul').html(mapstr);
	if(role.guajimapId){
		$("#mapbtn"+role.guajimapId).addClass('mychujibutton');
		$("#mapbtn"+role.guajimapId).text("结算");
	}
	$('#roleguajijy').html(maps[role.guajimapId].jnjy);
}
...
function showjnlist(){
	mui.get('getMyjns',function(data){
		if(data){
			var jnliststr = "";
			for(var i=0;i<data.data.length;i++){
				jnliststr = jnliststr + "<img src='"+data.data[i].touxiang+"' class='jntxlist' style='border: 3px "+data.data[i].color+" solid' onclick='showjninfo2("+data.data[i].id+")'/> ";
			}
			$('#jnlistdiv').empty();
	...
	$.ajax({
		...
		success: function (data) {
				if(data){
					...
					var spliststr = "";
					for(var i=0;i<data.data.length;i++){
						if (data.data[i].name == "泛用型布里"){
							zblnum = data.data[i].num;
						} else if (data.data[i].name == "试作型布里MKII"){
							jblnum = data.data[i].num;
						} else{				
							spliststr = spliststr + "<li id='splistid"+data.data[i].id+"' class='mui-table-view-cell mui-media'><div style='float: left;'><img class='mui-media-object mui-pull-left' src='"+data.data[i].touxiang+"'><div class='mui-media-body'><font color='"+data.data[i].color+"'>"+data.data[i].name+"</font><p class='myp mui-ellipsis'>数量：<span>"+data.data[i].num+"</span>/<span>"+data.data[i].spnum+"</span></p></div></div><div style='float: right;'><div id='numbox"+data.data[i].id+"' class='mui-numbox' data-numbox-step='1' data-numbox-min='0' data-numbox-max='"+data.data[i].num+"'><button class='mui-btn mui-numbox-btn-minus' type='button'>-</button><input class='mui-numbox-input' type='number' /><button class='mui-btn mui-numbox-btn-plus' type='button'>+</button></div> <button type='button' class='imglibutton mui-btn' onclick='salesuipian("+data.data[i].id+","+data.data[i].pinji+")'>出售</button> <button type='button' class='imglibutton mui-btn' onclick='jnhecheng("+data.data[i].id+","+data.data[i].spnum+")'>合成</button></div></li>";
						}
					}
					$('#zbl').html(zblnum);
					$('#jbl').html(jblnum);
...
//	location.href="#cangkuview";
//	mui.get('getSps',function(data){
//		if(data){
//			var zblnum = 0;
//			var jblnum = 0;
//			var spliststr = "<ul class='mui-table-view' id='splistul'>";
//			for(var i=0;i<data.data.length;i++){
//				if (data.data[i].name == "泛用型布里"){
//					zblnum = data.data[i].num;
//				} else if (data.data[i].name == "试作型布里MKII"){
...
function mapguaji(id){
	...
	mui.post(domain+'mapguaji/',{'id':id},function(data){
		if(data.result=='success'){
			...
			if(data.obtsp){
				for(var i =0 ;i<data.obtsp.length;i++){
					str = str + "<font color='"+data.obtsp[i].color+"'>"+data.obtsp[i].name+"碎片、</font>";
				}
			}
			mui.toast(str,{ duration:'long', type:'div' });
			getRoleDataWithoutMap();
